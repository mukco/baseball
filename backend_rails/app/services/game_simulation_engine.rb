class GameSimulationEngine
  ...
  class << self
    ...
    # -------------------------------------------------------------------------
    def simulate_game(home_lineup:, away_lineup:, home_pitchers:, away_pitchers:, blend: 0.45)
      batter_stats  = {}
      pitcher_stats = {}
      ...
      (1..MAX_INNINGS).each do |inning|
        ...
        home_pitch_idx = maybe_pull_pitcher(home_pitchers, home_pitch_idx, pitcher_stats)
        ...
        unless inning >= 9 && home_score > away_score
          ...
          away_pitch_idx = maybe_pull_pitcher(away_pitchers, away_pitch_idx, pitcher_stats)
        end
    ...
    def simulate_half_inning(lineup:, pitcher:, bat_pos:, batter_stats:, pitcher_stats:, blend:)
      outs  = 0
      bases = [nil, nil, nil]
      runs  = 0
      pos   = bat_pos
      ...
      while outs < 3
        ...
        outcome = simulate_pa(br, pr, blend)
        pitcher_stats[ppid][:bf] += 1

        case outcome
        when :walk
          batter_stats[pid][:bb] += 1
          scored, bases = apply_walk(bases, pid)
          runs += scored
        ...
        when :single
          batter_stats[pid][:ab] += 1
          batter_stats[pid][:h]  += 1
          scored, rbis, bases = apply_single(bases, pid)
        ...
        when :double
          batter_stats[pid][:ab] += 1
          batter_stats[pid][:h]  += 1
          scored, rbis, bases = apply_double(bases, pid)
          batter_stats[pid][:rbi] += rbis
          runs += scored
          pitcher_stats[ppid][:h]  += 1
          pitcher_stats[ppid][:er] += scored
    ...
    def simulate_pa(batter_rates, pitcher_rates, blend)
      ...
      babip     = clamp(r(batter_rates, :babip,   db) * br + r(pitcher_rates, :babip,  dp) * pr, 0.200, 0.380)
      hr_fb_pct = clamp(r(batter_rates, :hr_fb_pct, db) * br + r(pitcher_rates, :hr_fb_pct, dp) * pr, 0.02, 0.32)
    ...
    def apply_single(bases, batter_id)
      ...
      if bases[2];  runs += 1; rbis += 1; end

      if bases[1]
        if rand < SINGLE_SCORE_FROM_2B;  runs += 1; rbis += 1
      ...
      [runs, rbis, [new_first, new_two, new_three]]
    end
    ...
    def apply_double(bases, batter_id)
      ...
      if bases[2];  runs += 1; rbis += 1; end
      if bases[1];  runs += 1; rbis += 1; end

      if bases[0]
        if rand < DOUBLE_SCORE_FROM_1B;  runs += 1; rbis += 1
        else;                             new_three = bases[0]
        end
      end

      [runs, rbis, [nil, batter_id, new_three]]
    end
    ...
    def blank_pitcher_stat
      { bf: 0, outs: 0, h: 0, er: 0, bb: 0, k: 0, decision: nil }
    end
