class SimulateSeasonJob < ApplicationJob
  queue_as :default
  ...
  def perform(job_run_id)
    ...
    result = SimulationService.simulate_season(league, job_run: job_run)
  ...
  def enqueue_news_for_new_dates(league, already_simulated)
    ...
    Rails.logger.warn "[SimulateSeasonJob] news enqueue failed: #{e.message}"
  end
end
