const STAT_HELP = {
  ...
  ppd: {
    label: 'PPD',
    definition: 'Points Per Dollar — FanGraphs Points produced divided by Ottoneu salary. The core Ottoneu value metric.',
    formula: 'FG Points / Salary.',
    formulaLatex: '\\mathrm{PPD}=\\frac{\\text{FG Points}}{\\text{Salary}}',
    interpretation: '10 PPD = fair value (the league average return per dollar). Higher is better.',
    intuition: 'Think of it as return on investment — a $5 player scoring 75 pts (15 PPD) beats a $30 player scoring 210 pts (7 PPD).',
  },
  surplus: {
    label: 'Surplus $',
    definition: 'Surplus value in dollars — how much production you are getting above or below what the salary "should" buy at fair value (10 PPD).',
    formula: 'Implied Salary − Actual Salary, where Implied Salary = FG Points / 10.',
    formulaLatex: '\\text{Surplus}=\\frac{\\text{FG Points}}{10}-\\text{Salary}',
    interpretation: 'Positive = above fair value (getting a bargain). Negative = below fair value (overpaying).',
    intuition: 'Think of it as the dollar gap between what a player is worth and what you pay — +$20 means you are getting $20 of free production relative to market.',
  },
  approxFgPts: {
    label: 'Pts',
    definition: 'Ottoneu FanGraphs Points scored this season, calculated from warehouse counting stats using the full H2H FG Points formula.',
    formula: 'Batters: AB×(−1) + H×5.6 + 2B×2.9 + 3B×5.7 + HR×9.4 + BB×3.0 + HBP×3.0 + SB×1.9 + CS×(−2.8). Pitchers: IP×7.4 + K×2.0 + H×(−2.6) + BB×(−3.0) + HBP×(−3.0) + HR×(−12.3) + SV×5.0 + HLD×4.0.',
    interpretation: 'Higher is better. Season-to-date total, not a rate stat.',
    intuition: 'Think of it as the raw points production a player has delivered — the numerator in PPD.',
  },
  fairValueSalary: {
    label: 'Fair $',
    definition: 'The maximum salary at which a player delivers exactly fair value (10 PPD) based on their season-to-date production.',
    formula: '~Pts / 10',
    formulaLatex: '\\text{Fair\\$}=\\frac{\\widetilde{\\text{Pts}}}{10}',
    interpretation: 'Bid at or below this number to get fair value or better. Bidding above it means you are paying a premium.',
    intuition: 'Think of it as the ceiling bid — $1 above this and the player is technically overpriced at current production.',
  },
  projectedPts: {
    label: 'Proj Pts',
    definition: 'Full-season FanGraphs Points projected by Steamer, calculated from projection-table counting stats using Ottoneu H2H FG Points scoring.',
    formula: 'Same formula as ~Pts, applied to projected PA/IP and counting stats.',
    interpretation: 'Use this alongside ~Pts to judge whether current production is sustainable. Compare to fair_value_salary for a bid ceiling.',
    intuition: 'Think of it as what the market is pricing in — if Proj Pts is much higher than season-to-date, the player may be injured or in a slump.',
  },
  vsProjection: {
    label: 'vs Proj',
    definition: 'How many FanGraphs Points ahead or behind a player is versus their full-season projection, scaled to their projected playing time.',
    formula: 'Batters: (actual_pts / actual_PA × proj_PA) − proj_pts. Pitchers: (actual_pts / actual_IP × proj_IP) − proj_pts.',
    formulaLatex: '\\text{vs Proj}=\\frac{\\text{Actual Pts}}{\\text{Actual PA}}\\times\\text{Proj PA}-\\text{Proj Pts}',
    interpretation: 'Positive = outperforming projection (potential buy-low before market adjusts). Negative = underperforming.',
    intuition: 'Think of it as pace vs expectation — a +30 means the player is on track to beat his projection by 30 pts at current rate.',
  },
}

const STAT_ALIASES = {
  // Ottoneu fantasy value metrics
  ppd: 'ppd',
  ...
  '~pts': 'approxFgPts',
  pts: 'approxFgPts',
  approxfgpts: 'approxFgPts',
  'approx fg pts': 'approxFgPts',
  'fg pts': 'approxFgPts',
  'fair $': 'fairValueSalary',
  'fair$': 'fairValueSalary',
  fairvaluesalary: 'fairValueSalary',
  'fair value salary': 'fairValueSalary',
  'proj pts': 'projectedPts',
  projectedpts: 'projectedPts',
  'projected pts': 'projectedPts',
  'vs proj': 'vsProjection',
  vsprojection: 'vsProjection',
  'vs projection': 'vsProjection',
  // Standard keys
  'wrc+': 'wrcPlus',
  war: 'war',
  avg: 'avg',
